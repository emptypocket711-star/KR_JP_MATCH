import { createHash } from 'crypto';

/**
 * Immutable DER certificates downloaded from Apple's public PKI repository.
 *
 * The base64 payloads are committed with their SHA-256 fingerprints so a
 * corrupted or accidentally replaced trust anchor fails closed at startup.
 * Updating these assets must be a reviewed source change, never a runtime
 * network fetch.
 */
export const appleRootCertificateManifest = [
  {
    name: 'Apple Inc. Root',
    sourceUrl: 'https://www.apple.com/appleca/AppleIncRootCertificate.cer',
    sha256: 'b0b1730ecbc7ff4505142c49f1295e6eda6bcaed7e2c68c5be91b5a11001f024',
    derBase64: [
      '',
    ].join(''),
  },
  {
    name: 'Apple Root CA - G2',
    sourceUrl: 'https://www.apple.com/certificateauthority/AppleRootCA-G2.cer',
    sha256: 'c2b9b042dd57830e7d117dac55ac8ae19407d38e41d88f3215bc3a890444a050',
    derBase64: [
      '',
    ].join(''),
  },
  {
    name: 'Apple Root CA - G3',
    sourceUrl: 'https://www.apple.com/certificateauthority/AppleRootCA-G3.cer',
    sha256: '63343abfb89a6a03ebb57e9b3f5fa7be7c4f5c756f3017b3a8c488c3653e9179',
    derBase64: [
      '',
    ].join(''),
  },
] as const;

export function loadPinnedAppleRootCertificates(): Buffer[] {
  return appleRootCertificateManifest.map((asset) => {
    const certificate = Buffer.from(asset.derBase64, 'base64');
    const actualHash = createHash('sha256').update(certificate).digest('hex');
    if (actualHash !== asset.sha256) {
      throw new Error(`Pinned Apple root certificate hash mismatch: ${asset.name}`);
    }
    return certificate;
  });
}
